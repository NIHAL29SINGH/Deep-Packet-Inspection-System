...
## Java Port - Recent Updates (2026-02-21)

The Java version in `java-port` was updated to make day-to-day usage simpler and safer.

### What Changed

1. Added command shortcuts in `java-port`:
   - `./build.cmd`
   - `./normal.cmd`
   - `./block.cmd <rules>`
   - `./live.cmd <seconds|Xm|Xs>`
   - `./csv.cmd <name.csv> [input.pcap]`
   - `./report.cmd <name> [input.pcap]`

2. Added report exports:
   - CSV export via `--report-csv <file>`
   - JSON export via `--report-json <file>`

3. Added combined report mode:
   - `./report.cmd mycheck` creates:
     - `output_mycheck.pcap`
     - `mycheck.csv`
     - `mycheck.json`

4. Cleaned console output:
   - Replaced garbled Unicode box characters with plain ASCII formatting for better PowerShell compatibility.

5. Unified generated files into one folder:
   - All generated `.pcap`, `.csv`, and `.json` now go to:
     - `java-port/outputs`
   - Folder is auto-created if deleted.

6. Live mode behavior:
   - Live flow is capture + DPI processing only.
   - No firewall/live-enforcement blocking logic.

### Quick Start (Java Port)

```powershell
cd "D:\deep packet\Packet_analyzer-main\java-port"
.\build.cmd
.\normal.cmd
.\block.cmd youtube,facebook,telegram
.\live.cmd 60
.\csv.cmd output_blocked.csv
.\report.cmd mycheck
